...
//Read input informations
  ...
  bins_size = (box/2)/nbins;

  for(int i=0;i<nbins;++i) {

    delta_v[i] = 4*pi/3 * ( pow( bins_size*(i+1),3 ) - pow( bins_size*i,3) );
  ...
  u_corr = 8 * pi * rho * ( 1./(9*pow(rcut,9)) - 1./(3*pow(rcut,3)) );
  p_corr = 32 * pi * rho* ( 1./(9*pow(rcut,9)) - 1./(6*pow(rcut,6)) );
...
//Prepare arrays for measurements
  ...
  for(int i=0;i<nbins;++i) ig[i] = i+5;

  n_props = nbins+5; //Number of observables
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        w += 48*( 1.0/pow(dr,12) - 0.5* 1.0/pow(dr,6));
      }

      //g(r) computation here
      if(dr < box/2)  g[static_cast<int>(dr/bins_size)] += 2;
  ...
  walker[ip] = rho*walker[it] + 1/(3*vol)*w/(double)npart; // Pressure

  for(int i=0;i<nbins;++i) walker[ig[i]] = g[i]/(rho * m_part * delta_v[i]);
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, G;
   const int wd=12;
    ...
    for(int i=0; i<nbins;++i) {
      ...
      stima_g[i] = blk_av[ig[i]]/blk_norm;
      glob_av[ig[i]] += stima_g[i];
      glob_av2[ig[i]] += stima_g[i]*stima_g[i];
      err_g[i] = Error(glob_av[ig[i]], glob_av2[ig[i]],iblk);

    }

//Potential energy per particle
    Epot << setw(wd) << iblk <<  setw(wd) << stima_pot + u_corr << setw(wd) << (glob_av[iv] + u_corr)/(double)iblk << setw(wd) << err_pot << endl;
//Kinetic energy
    Ekin << setw(wd) << iblk <<  setw(wd) << stima_kin << setw(wd) << glob_av[ik]/(double)iblk << setw(wd) << err_kin << endl;
//Total energy
    Etot << setw(wd) << iblk <<  setw(wd) << stima_etot + u_corr << setw(wd) << (glob_av[ie] + u_corr)/(double)iblk  << setw(wd) << err_etot << endl;
//Temperature
    Temp << setw(wd) << iblk <<  setw(wd) << stima_temp << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_temp << endl;
//Pressure
    Pres << setw(wd) << iblk <<  setw(wd) << stima_pres - p_corr << setw(wd) << (glob_av[ip] - p_corr)/iblk << setw(wd) << err_pres << endl;

    for(int i=0;i<nbins;++i) G << iblk << setw(wd) << stima_g[i] << " ";

    G << endl;

    if(iblk == nblk) {

      ofstream G_R("output_gdir.dat");

      for(int i=0;i<nbins;++i)  G_R << i*bins_size << setw(wd) << glob_av[ig[i]]/(double)iblk << setw(wd) << err_g[i] << endl;
