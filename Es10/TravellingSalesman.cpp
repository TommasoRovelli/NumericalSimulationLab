...
TravellingSalesman::TravellingSalesman(int line) {m_rnd.Init(line);}
...
TravellingSalesman::TravellingSalesman(const std::string &filename) {
    ...
    m_N_c = static_cast<unsigned int>(m_cities.size());
 
}

TravellingSalesman::TravellingSalesman(const std::string &filename,int line) {
...
void TravellingSalesman::SetCities(std::vector<City> cities) {m_cities=cities;}

std::vector<Path>& TravellingSalesman::GetPopulation() { return m_population;}

void TravellingSalesman::Shuffle(std::vector<City> &c) {

    for (int i=1; i<(int)(c.size()-1); ++i) {
    std::swap(c[i],c[m_rnd.RndInt(i-1)+1]);
    }
...
unsigned int& TravellingSalesman::GetCitiesNum() { return m_N_c;}
